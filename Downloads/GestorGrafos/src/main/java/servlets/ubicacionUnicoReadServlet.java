...
@WebServlet(name = "ubicacionUnicoReadServlet", urlPatterns = {"/ubicacionUnicoReadServlet"})
public class ubicacionUnicoReadServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Session session = GraphDatabase.driver(NEO4J_URI, AuthTokens.basic(NEO4J_USER, NEO4J_PASSWORD)).session()) {
            String query = "MATCH (u:Ubicación {pais: $pais}) RETURN u.pais AS nombre";
            Result result = session.run(query, org.neo4j.driver.Values.parameters("pais", pais));

            if (result.hasNext()) {
                var record = result.next();
                nombrePais = record.get("nombre").asString();
            }
        } catch (Exception e) {
            e.printStackTrace();
        ...
        request.getRequestDispatcher("ubicacionUnicoRead.jsp").forward(request, response);
    }
    ...
    public String getServletInfo() {
        return "Servlet para leer una ubicación única";
    }
}